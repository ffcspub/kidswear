//
//  UserIconSettingViewController.m
//  flowerong
//
...
@interface UserIconSettingViewController (){
    MBProgressHUD *_hud;
...
@implementation UserIconSettingViewController
...
- (id)initWithImage:(UIImage *)originalImage cropFrame:(CGRect)cropFrame limitScaleRatio:(NSInteger)limitRatio {
    self = [super initWithNibName:@"UserIconSettingViewController" bundle:nil];
    if (self) {
...
- (void)initView
{
    ...
    [button addTarget:self action:@selector(handleFinish) forControlEvents:UIControlEventTouchUpInside];
...
//    self.ratioView = [[UIView alloc] initWithFrame:self.cropFrame];
//    [self.view addSubview:self.ratioView];
//    circleView *circle = [[circleView alloc] initWithFrame:CGRectMake(0, 0, self.cropFrame.size.width, self.cropFrame.size.height)];
//    [self.ratioView addSubview:circle];
...
- (void)handleFinish
{
    ...
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(UserIconSettingDelegate)]) {
        [self.delegate imageCropper:self didFinished:image];
    }
